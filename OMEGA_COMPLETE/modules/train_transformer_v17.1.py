# train_transformer_v17.py – Entrenamiento optimizado para LotteryTransformer (OMEGA PRO AI v12.1) – Versión con normalize, Y-M-D detect, L2
...
class LotteryLoss(nn.Module):
    def __init__(self, alpha=0.5):  # Tweak: 0.5 balance
        super().__init__()
...
def train_transformer(historial_path: str, epochs: int = 100, batch_size: int = 32, lr: float = 0.0001, seq_length: int = 10, val_split: float = 0.2, model_path: str = "model/transformer_model.pt", fine_tune: bool = True, dry_run: bool = False, force_cpu: bool = False):
    # Chequeo de archivo
    ...
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-5)  # Tweak: L2 reg
