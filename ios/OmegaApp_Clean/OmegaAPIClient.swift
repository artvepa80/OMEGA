...
class OmegaAPIClient: ObservableObject {
    ...
    private let sslErrorHandler = SSLErrorHandler.shared
    ...
    func login(username: String, password: String) async throws -> AuthResponse {
        ...
        do {
            ...
            // Handle SSL/Network security errors through our error handler
            return try await withCheckedThrowingContinuation { continuation in
                sslErrorHandler.handleNetworkError(networkError, for: url.host ?? baseURL) { resolution in
                    switch resolution {
                    case .retry:
                        Task {
                            do {
                                let result = try await self.login(username: username, password: password)
                    ...
                    case .cancel:
                        continuation.resume(throwing: APIError.networkError(networkError))
                    default:
                        continuation.resume(throwing: APIError.networkError(networkError))
                    }
                }
