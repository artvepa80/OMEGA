# === OMEGA Structured Logging Shim (auto-injected) ===
# This routes all print() calls through utils.logging.get_logger so logs are uniform.
try:
    from utils.logging import get_logger  # your centralized logger
    _omega_logger = get_logger("OMEGA.main")
except Exception:  # fallback if utils.logging isn't available yet
    import logging as _fallback_logging
    _fallback_logging.basicConfig(level=_fallback_logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
    _omega_logger = _fallback_logging.getLogger("OMEGA.main")

# Monkey-patch print -> logger.info to keep existing code unchanged
import builtins as _builtins
def _omega_print(*args, **kwargs):
    try:
        msg = " ".join(str(a) for a in args)
    except Exception:
        # Fallback to repr if str fails
        msg = " ".join(repr(a) for a in args)
    _omega_logger.info(msg)
_builtins.print = _omega_print
# === End shim ===
...
# ---------------------------------------------------------------------------
# 6. FUNCIONES DE IA AVANZADA (NUEVAS) Y UTILIDADES ASYNC
# ---------------------------------------------------------------------------
...
async def main(
    ...
    try:
        ...
        # Cargar datos con manejo específico de errores
        if ERROR_HANDLING_AVAILABLE:
            df_raw = safe_execute(
                pd.read_csv,
                data_path,
                context="Carga de datos CSV",
                fallback=None
            )
            
            if df_raw is None:
                raise FileIOError(
                    f"No se pudo cargar el archivo de datos: {data_path}",
                    file_path=data_path,
                    operation="read"
                )
        else:
        ...
        df_numbers = df_raw[cols].apply(pd.to_numeric, errors="coerce")
        
        # Reemplazar inf e infinitos con NaN antes de imputar
        df_numbers = df_numbers.replace([np.inf, -np.inf], np.nan)
        
        # Usar SimpleImputer para rellenar NaN con la media
        imputer = SimpleImputer(strategy="mean")
        df_imputed = imputer.fit_transform(df_numbers)
        
        # Convertir a DataFrame y asegurar enteros válidos
        historial_df = pd.DataFrame(df_imputed, columns=cols)
        
        # Clip a rango válido antes de convertir a int
        historial_df = historial_df.clip(1, 40).round().astype(int)
    ...
    if not dry_run and "json" in export_formats and config.get("export_json", False):
        ...
        try:
            ...
            with open(json_path, "w", encoding="utf-8") as fh:
                json_module.dump(export_data, fh, indent=4, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    # Ejecutar función principal async de manera segura
    asyncio.run(main())
