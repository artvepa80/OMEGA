# OMEGA_PRO_AI-v10.1/utils/transformer_data_utils.py – Versión Ultra-Mejorada con multi-format date detect (OMEGA PRO AI v12.2)
...
def prepare_advanced_transformer_data(df: pd.DataFrame, seq_length: int = 10, for_training: bool = False):
    ...
    # 2. Si no se encuentra, intentar parsear columnas de tipo objeto con múltiples formatos
    if not col_fecha:
        formats = ['%Y-%m-%d', '%d-%m-%Y', '%m/%d/%Y', '%Y/%m/%d']  # Common formats
        for c in df.columns:
            if pd.api.types.is_object_dtype(df[c]) and len(df) > 0:
                for fmt in formats:
                    try:
                        temp = pd.to_datetime(df[c], format=fmt, errors='raise')
                        col_fecha = c
                        print(f"✅ Detectada columna de fecha '{c}' con formato {fmt}")
                        logger.info(f"✅ Detectada columna de fecha '{c}' con formato {fmt}")
                        break
                    except ValueError:
                        pass
                if col_fecha:
    ...
    # 3. Advanced: Try without format, validate low NaT rate (<50%)
    if not col_fecha:
        for c in df.columns:
            if pd.api.types.is_object_dtype(df[c]) and len(df) > 0:
                try:
                    temp = pd.to_datetime(df[c], errors='coerce')
                    nat_rate = temp.isna().mean()
                    if nat_rate < 0.5:  # <50% NaT = likely date
                        col_fecha = c
                        print(f"✅ Detectada columna de fecha '{c}' por auto-parse (NaT rate {nat_rate*100:.1f}%)")
                        logger.info(f"✅ Detectada columna de fecha '{c}' por auto-parse (NaT rate {nat_rate*100:.1f}%)")
                        break
                except ValueError:
                    pass
    
    # 4. Si aún no se encuentra, crear una columna de fecha dummy
    if not col_fecha:
        logger.warning("⚠️ No se encontró columna de fecha. Creando fecha dummy.")
    ...
    # Normalize ano to 0-1 (e.g., for 2000-2100 range)
    min_ano = 2000
    max_ano = 2100
    df['ano'] = (df['ano'] - min_ano) / (max_ano - min_ano)
    ...
    # Asserts de shapes solicitados: (batch, seq_len, features)
    assert X_seq.ndim == 3 and X_seq.shape[1] == seq_length and X_seq.shape[2] == 6, "X_seq shape inválida"
    assert X_temp.ndim == 3 and X_temp.shape[1] == seq_length and X_temp.shape[2] == 3, "X_temp shape inválida"
    assert X_pos.ndim == 2 and X_pos.shape[1] == seq_length, "X_pos shape inválida"
