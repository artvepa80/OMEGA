...
SCORE_CONFIG = {
    ...
    'perfil_thresholds': {
        'moderado': 0.7,
        'conservador': 0.8,
        'agresivo': 0.4  # Ajustado para más salidas
    }
}
...
def calculate_diversity(combo: List[int], other_combos: List[List[int]]) -> float:
    ...
    return 1.0 - similarity  # IMPROVED: Use Jaccard for better lottery similarity measure
...
def score_combinations(
    ...
    - Bonificaciones por extremos y clusters
    ...
    if not historial.empty and all(col in historial.columns for col in historial_columns):
        historial = historial[historial_columns].dropna().astype(int)
    ...
    if filtro and rows > 0:
        try:
            ...
            historical_scores = Parallel(n_jobs=-1)(delayed(calc_score)(row) for row in historial.values[-100:])  # ADDED: Parallelize
            if historical_scores:  # FIXED: Check if not empty
                percentile_val = np.percentile(historical_scores, 25)
                threshold = max(threshold * 0.8, min(threshold, max(0, percentile_val)))  # Ensure non-negative
    ...
    # ADDED: Parallelize main scoring loop
    def score_single(cd, idx):
        if not isinstance(cd, dict) or 'combination' not in cd:
            ...
            return None, 1  # Return None for scored, +1 rejected
        ...
        # 🎯 5. Decades
        decades = np.minimum((np.array(sorted_combo) - 1) // 10, 3)
        ...
        if cluster_data is not None:  # ADDED: Check if None
        ...
        return {
            ...
            'metrics': metrics,
            'normalized': 0.0
        }, 0  # Return scored item, 0 rejected

    # Parallelize the main loop
    results = Parallel(n_jobs=-1)(delayed(score_single)(cd, idx) for idx, cd in enumerate(combinations))
    ...
    if scored:
        ...
        fb_decades = dict(Counter(min((n - 1) // 10, 3) for n in fb))
    ...
    log_dir = f"logs/{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}_{os.getpid()}"  # FIXED: Add PID to avoid collisions
    os.makedirs(log_dir, exist_ok=True)
    pd.DataFrame([item['metrics'] for item in scored]).to_csv(f"{log_dir}/score_metrics.csv", index=False)  # Use scored metrics
    ...
    summary = {
        ...
        'max_score': max([item['score'] for item in scored]) if scored else 0.0,  # ADDED: Max/min in summary
