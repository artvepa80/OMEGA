...
class DataSecurity:
    ...
    def apply_differential_privacy(self, data: np.ndarray, epsilon: float = 1.0) -> np.ndarray:
        ...
        if not PRIVACY_AVAILABLE:
            # Fallback: add basic Laplace noise
            sensitivity = np.max(data) - np.min(data) if len(data) > 0 else 1.0
...
def validate_data_security(data, config: Dict = None) -> Dict:
    """Wrapper function for data security validation"""
    try:
