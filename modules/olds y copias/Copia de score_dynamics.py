...
def clean_combination(combo, logger):
    """Convierte np.int64 a int en combinaciones."""
    try:
        return [int(n) if isinstance(n, np.integer) else n for n in combo]
...
def calculate_diversity(combo: List[int], other_combos: List[List[int]]) -> float:
    """Calculate diversity score based on Hamming distance from other combinations."""
    if not other_combos:
        return 1.0
    distances = [hamming(sorted(combo), sorted(other)) for other in other_combos]
...
def score_combinations(
    ...
    if not historial.empty and all(col in historial.columns for col in historial_columns):
        ...
        if valid_range:
            ...
            if not validate_combination(ultimos):
                logger.info(f"⚠️ Último sorteo inválido: {ultimos}, se omite")
                ultimos = []
        else:
    ...
    if filtro and rows > 0:
        try:
            historical_scores = [filtro.aplicar_filtros(row, return_score=True, perfil_svi=perfil_svi)[0] 
                                for row in historial.values[-100:]]
            if historical_scores:
                threshold = max(threshold * 0.8, min(threshold, np.percentile(historical_scores, 25)))
    ...
    log_dir = f"logs/{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(log_dir, exist_ok=True)
    pd.DataFrame(metrics_log).to_csv(f"{log_dir}/score_metrics.csv", index=False)
