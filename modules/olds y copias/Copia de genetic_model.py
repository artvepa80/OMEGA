...
class GeneticModel:
    ...
    def _get_combo_info(self, combo: List[int]) -> Dict:
        ...
        decades = set((num - 1) // 10 for num in combo)
    ...
    def mutate(self, individual: List[int], diversity: float) -> List[int]:
        ...
        for num in options:
            if num not in self.recent_performance:
                self.recent_performance[num] = 1.5
            else:
...
def generar_combinaciones_geneticas(
    data: pd.DataFrame,
    historial_set: Set[Tuple[int, ...]],  # FIX: Cambiado de 'historial' a 'historial_set' para coincidir con llamadas
    cantidad: int = 30,
    config: GeneticConfig = GeneticConfig(),
    logger: Optional[logging.Logger] = None
) -> List[Dict]:
    ...
    try:
        ...
        for combo in population:
            ...
            if combo_tuple in seen or combo_tuple in historial_set:  # FIX: Usar 'historial_set'
                continue
        ...
        if len(results) < cantidad:
            ...
            while backup_count < needed and max_backup_attempts > 0:
                ...
                if (candidate_tuple not in seen and 
                    candidate_tuple not in historial_set and  # FIX: Usar 'historial_set'
                    model._apply_filter(candidate, historial_set) and  # FIX: Usar 'historial_set'
        ...
        try:
            ...
            if not numeric_data.empty:
                scored = score_combinations(combo_list, numeric_data)
        ...
        valid_results = validate_results(results, historial_set)  # FIX: Usar 'historial_set'
        ...
        return generate_fallback(cantidad, historial_set, config.seed, logger)  # FIX: Usar 'historial_set'

def validate_results(results: List[Dict], historial_set: Set[Tuple[int, ...]]) -> List[Dict]:  # FIX: Cambiado a 'historial_set'
    valid_results = []
    seen = set()
...
def generate_fallback(
    cantidad: int, 
    historial_set: Set[Tuple[int, ...]],  # FIX: Cambiado a 'historial_set'
    seed: Optional[int] = None,
    logger: Optional[logging.Logger] = None
) -> List[Dict]:
    fallback = []
    ...
    while len(fallback) < cantidad and fallback_attempts < cantidad * 15:
        ...
        if (candidate_tuple not in historial_set and  # FIX: Usar 'historial_set'
            valid_parity and valid_decades and no_concentration):
