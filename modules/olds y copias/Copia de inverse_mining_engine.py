...
default_logger = logging.getLogger("InverseMining")
...
def validar_entrada(seed, boost, penalize, focus_positions, logger=default_logger):
    ...
    if not validate_combination(seed):
        logger.error(f"🚨 Invalid seed: {seed}. Must be 6 unique integers between 1-40")
    ...
    for pos in focus_positions:
        if pos.upper() in valid_positions:
            ...
            logger.info(f"⚠️ Invalid position ignored: {pos}")
...
def aplicar_mineria_inversa(historial_df: pd.DataFrame, seed, boost, penalize, focus_positions, cantidad, perfil_svi, logger=default_logger):
    ...
    try:
        ...
        # FIX: Removed logger parameter to match simulate_ghost_rng signature
        ...
        for sim in simuladas:
            ...
            if not validate_combination(combo):
                logger.info(f"⚠️ Invalid combination skipped: {combo}")
                continue
            ...
            if score_final >= umbral:
                ...
                logger.info(f"🧹 Rejected combination: {combo}, Score: {score_final:.4f}, Reasons: {razones}")

        resultados.sort(key=lambda x: x[1], reverse=True)
        return resultados[:min(cantidad, MAX_RESULTS)]

    except Exception as e:
        logger.error(f"🚨 Error in aplicar_mineria_inversa: {str(e)}")
        return []
...
def mostrar_resumen(resultados, boost, penalize, focus_positions, logger=default_logger):
    ...
    logger.info("\n" + "="*60)
    logger.info(f"🔥 Resumen de Minería Inversa")
    logger.info(f"📊 Combinaciones generadas: {len(resultados)}")
    logger.info(f"💎 Mejor score: {resultados[0]['score']:.4f}")
    logger.info(f"⚙️ Parámetros aplicados:")
    logger.info(f"   - Boost: {boost or 'Ninguno'}")
    logger.info(f"   - Penalize: {penalize or 'Ninguno'}")
    logger.info(f"   - Posiciones estratégicas: {focus_positions or 'Ninguna'}")
    logger.info("="*60)
...
def main():
    ...
    if not args.silent:
        logger.info("\n" + "🚀"*30)
        logger.info("🔍 EJECUTANDO MINERÍA INVERSA AVANZADA - OMEGA PRO AI v10.10")
        logger.info("🚀"*30)
    ...
    try:
        ...
        if not all(historial_df[col].between(1, 40).all() for col in expected_cols):
            logger.error("🚨 Invalid historical data: numbers outside range [1, 40]")
            sys.exit(1)
