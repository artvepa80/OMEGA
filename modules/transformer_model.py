...
def get_adaptive_transformer_config(resources: Dict) -> Dict:
    ...
    config = {
        ...
        'd_model': 128,  # Fixed: align with saved model architecture
...
def cargar_modelo_transformer(model_path="model/transformer_model.pt", train_if_missing=False, 
    ...
    try:
        ...
        model = LotteryTransformer(
            num_numbers=40, 
            d_model=128,  # Fixed: force consistent architecture with saved model 
        ...
        logger.info(f"Transformer configurado: d_model=128 (fixed architecture), "
        ...
        if os.path.exists(model_path):
            try:
                ...
                # Handle architecture mismatch by loading compatible keys only
                ...
                for key, value in state_dict.items():
                    if key in model_dict and model_dict[key].shape == value.shape:
                        ...
                        logger.warning(f"⚠️ Skipping incompatible key: {key} (saved: {value.shape if hasattr(value, 'shape') else 'N/A'}, current: {model_dict.get(key, 'N/A').shape if key in model_dict and hasattr(model_dict[key], 'shape') else 'N/A'})")
                ...
                logger.info(f"✅ Modelo Transformer parcialmente cargado desde {model_path} ({loaded_keys}/{total_keys} capas compatibles)")
                
                # If very few weights were loaded, better to retrain
                if loaded_keys < total_keys * 0.5:
                    logger.warning("⚠️ Menos del 50% de pesos compatibles, reentrenando...")
                    if train_if_missing and historial_df is not None and not historial_df.empty:
                        return train_transformer_model(model, historial_df, device, logger, model_path)
    ...
    if historial_df is not None and combinaciones:
        try:
            combinaciones_scored = score_combinations(
                ...
                sequential=True  # Force sequential processing to avoid DataFrame serialization issues
            )
