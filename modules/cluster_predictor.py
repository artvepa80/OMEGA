...
# ✅ NUEVO: función usada por el predictor
def generar_combinaciones_clustering(historial, cantidad=60):
    import pandas as pd
    import random
    from modules.clustering_model import apply_clustering
    ...
    while len(combinaciones) < cantidad:
        centroide = random.choice(centroids)
        variacion = sorted(random.sample(universo, 6))
        if tuple(variacion) not in historial:
            combinaciones.append(variacion)
...
if __name__ == "__main__":
    ...
    # Evita ejecución de main si se importa
    pass