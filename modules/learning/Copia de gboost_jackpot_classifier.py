...
class GBoostJackpotClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _extract_features(self, combos_series: pd.Series) -> pd.DataFrame:
        ...
        try:
            # 1. Binarizar presencia de números (con fitted check)
            if not hasattr(self.mlb, 'classes_'):
                raise AttributeError("MultiLabelBinarizer no está ajustado. Llama a fit() primero.")
...
def predecir_perfil(modelo, combinaciones: list) -> list:
    """
    Devuelve perfiles simbólicos ('A', 'B') para cada combinación
    según la predicción del modelo (0 → B, 1 → A).
    
    Parámetros:
        modelo (GBoostJackpotClassifier): Modelo entrenado
        combinaciones (list): Lista de combinaciones a predecir
        
    Retorna:
        list: Lista de perfiles ('A' o 'B') para cada combinación
    """
    try:
        predicciones = modelo.predict(combinaciones)
...
if __name__ == "__main__":
    ...
    try:
        ...
        perfiles = predecir_perfil(clf, test_combos)
