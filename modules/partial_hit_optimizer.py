...
class PartialHitOptimizer:
    ...
    def get_optimization_suggestions(self) -> Dict:
        """Genera sugerencias de optimización para hits parciales"""
        
        suggestions = {
            'strategy': 'partial_hit_optimization',
            'target_hits': '4-5_numbers',
            'recommended_numbers': [],
            'avoid_numbers': [],
            'pattern_insights': {},
            'optimization_tips': [],
        ...
        try:
            # Analizar números con mejor co-ocurrencia
            best_co_occurrence = {}
            for num1 in range(1, 41):
                if num1 in self.number_co_occurrence:
                    total_score = sum(self.number_co_occurrence[num1].values())
                    avg_score = total_score / len(self.number_co_occurrence[num1]) if self.number_co_occurrence[num1] else 0
                    best_co_occurrence[num1] = avg_score
            
            # Números recomendados (top 15 por co-ocurrencia)
            sorted_by_cooccurrence = sorted(best_co_occurrence.items(), key=lambda x: x[1], reverse=True)
            suggestions['recommended_numbers'] = [num for num, score in sorted_by_cooccurrence[:15]]
            
            # Números a evitar (bottom 10 por co-ocurrencia)
            suggestions['avoid_numbers'] = [num for num, score in sorted_by_cooccurrence[-10:]]
            
            # Insights de patrones más frecuentes
            if self.partial_patterns:
                top_patterns = sorted(self.partial_patterns.items(), key=lambda x: x[1], reverse=True)[:5]
                suggestions['pattern_insights'] = {
                    'most_frequent_patterns': [
                        {
                            'pattern': list(pattern),
                            'frequency': freq,
                            'probability': freq / len(self.historical_data)
            ...
            suggestions['optimization_tips'] = [
                "Focus on numbers with high co-occurrence rates",
                "Use patterns that have appeared in at least 2% of historical draws",
                "Maintain balanced distribution across low/medium/high ranges",
                "Target 4-number hits for consistent returns",
                "Avoid over-concentration on any single number"
            ]
            
            # Análisis de distribución de rangos
            range_analysis = self.range_clusters
            if range_analysis:
                # Encontrar distribución más exitosa
                best_distribution = max(range_analysis.items(), key=lambda x: len(x[1]))
                suggestions['pattern_insights']['best_range_distribution'] = {
                    'pattern': best_distribution[0],
                    'frequency': len(best_distribution[1]),
                    'examples': best_distribution[1][:3]
                }
            
            # Calcular score de confianza
            confidence_factors = []
            
            if len(self.partial_patterns) > 0:
                confidence_factors.append(0.3)  # Tenemos patrones parciales
            
            if len(self.number_co_occurrence) > 20:
                confidence_factors.append(0.3)  # Buena cobertura de co-ocurrencia
            
            if len(self.range_clusters) > 0:
                confidence_factors.append(0.2)  # Tenemos análisis de clusters
            
            if len(self.historical_data) >= 100:
                confidence_factors.append(0.2)  # Datos suficientes
            
            suggestions['confidence_score'] = sum(confidence_factors)
            
            logger.info(f"🎯 Sugerencias de optimización generadas")
            logger.info(f"   Números recomendados: {suggestions['recommended_numbers'][:8]}")
            logger.info(f"   Score de confianza: {suggestions['confidence_score']:.1%}")
