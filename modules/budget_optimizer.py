...
def optimize_budget_allocation(historial_df: pd.DataFrame, num_combinaciones: int = 5) -> Dict:
    """Función wrapper para optimización de presupuesto"""
    try:
        optimizer = OmegaBudgetOptimizer(historial_df)
        
        # Generar recomendación estándar
        ...
        return {
            'budget_strategy': recommendation['recommended_strategy'],
            'monthly_cost': recommendation['monthly_cost'],
            'expected_roi': recommendation['roi_projection'].get('roi_percentage', 150),
            'combinations_recommended': num_combinaciones,
            'allocation': {
                'simple': 0.6,
                'complex': 0.4
            }
