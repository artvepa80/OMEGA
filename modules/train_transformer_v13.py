# train_transformer_v13.py – Entrenamiento optimizado para LotteryTransformer (OMEGA PRO AI v12.1) – Versión con BCE multi-label fix
...
class LotteryLoss(nn.Module):
    def __init__(self, alpha=0.7):
        super().__init__()
        self.num_loss = nn.BCEWithLogitsLoss()  # FIX: Multi-label BCE for 6 numbers
        self.sum_loss = nn.MSELoss()
        self.alpha = alpha
    
    def forward(self, pred_numbers, pred_sum, target_multi, target_sum):
        num_loss = self.num_loss(pred_numbers, target_multi.float())  # (batch,40) logits vs multi-hot
...
def train_transformer(historial_path: str, epochs: int = 100, batch_size: int = 32, lr: float = 0.001, seq_length: int = 10, val_split: float = 0.2, model_path: str = "model/transformer_model.pt", fine_tune: bool = True, dry_run: bool = False, force_cpu: bool = False):
    ...
    target_sums = torch.sum(targets, dim=1)  # Sumas
    
    # Multi-hot targets for BCE: (n,40), 1 in positions of the 6 numbers
    target_multi = torch.zeros((len(targets), 40))
    for i in range(len(targets)):
        target_multi[i, targets[i]] = 1  # Set 1 for each number (ignore duplicates if any)
    ...
    train_multi, val_multi = target_multi[:split_idx], target_multi[split_idx:]
    train_sums, val_sums = target_sums[:split_idx], target_sums[split_idx:]
    
    train_dataset = LotteryDataset(train_seq, train_temp, train_pos, train_multi, train_sums)  # Use multi-hot
    val_dataset = LotteryDataset(val_seq, val_temp, val_pos, val_multi, val_sums)
    ...
    criterion = LotteryLoss(alpha=0.7)  # FIX: Define criterion aquí
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            target_multi = batch['target_numbers'].to(device)  # Multi-hot (batch,40)
            target_sum = batch['target_sum'].to(device)
            
            optimizer.zero_grad()
            pred_numbers, pred_sum = model(numbers, temporal, positions)
            loss = criterion(pred_numbers, pred_sum, target_multi, target_sum)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                temporal = batch['temporal'].to(device)
                positions = batch['positions'].to(device)
                target_multi = batch['target_numbers'].to(device)
                target_sum = batch['target_sum'].to(device)
