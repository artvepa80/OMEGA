...
@dataclass
class GeneticConfig:
    pop_size: int = 30  # Reduced from 50 for faster execution
    generations: int = 10  # Reduced from 20 for faster execution  
    tournament_size: int = 3
    elite_fraction: float = 0.1
    mutation_rate: float = 0.25
    max_mutation_attempts: int = 25  # Reduced from 50
    ...
    early_convergence_threshold: float = 0.001  # Stop if improvement is minimal
...
class GeneticModel:
    ...
    def run(self) -> List[List[int]]:
        ...
        for generation in range(self.cfg.generations):
            # Check timeout
            if time.time() - start_time > self.cfg.timeout_seconds:
                self.logger.warning(f"🕒 Genetic algorithm timeout after {self.cfg.timeout_seconds}s at generation {generation}")
                break
            ...
            # Track best fitness for early convergence
            best_fitness_history.append(best)
            
            # Early convergence check
            if (generation >= 3 and 
                len(best_fitness_history) >= 3 and
                abs(best_fitness_history[-1] - best_fitness_history[-3]) < self.cfg.early_convergence_threshold):
                self.logger.info(f"🎯 Early convergence detected at generation {generation} (improvement < {self.cfg.early_convergence_threshold})")
                break
