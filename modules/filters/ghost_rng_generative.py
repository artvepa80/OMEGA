...
def passes_structural_filters(comb: Tuple[int, ...]) -> bool:
    """Verifica reglas básicas de estructura - versión más permisiva"""
    nums = sorted(comb)
    saltos = [b - a for a, b in zip(nums, nums[1:])]
    pares = sum(1 for n in nums if n % 2 == 0)
    variedad_decenas = len(set(n // 10 for n in nums))
    
    # Relaxed constraints to prevent over-filtering
    return all([
        1 <= pares <= 5,  # More permissive range
        2 <= variedad_decenas <= 6,  # More permissive range
        all(1 <= s <= 25 for s in saltos)  # Allow larger jumps
    ])
...
def get_seeds(
    ...
    try:
        ...
        if not combinaciones_filtradas:
            log_error("🚨 Filtros estructurales demasiado restrictivos, usando combinaciones válidas sin filtro estructural")
            # If structural filtering is too restrictive, use valid combinations without structural filter
            combinaciones_filtradas = combinaciones_validas[:max_seeds*2]  # Take more combinations
            
            # If still no combinations, generate fallback
            if not combinaciones_filtradas:
