...
class FiltroEstrategico:
    ...
    PRUEBAS_CONFIG = {
        ...
        'decadas': {
            'fn': lambda self, comb: (
                len(CollectionsCounter(min((n - 1) // 10, 3) for n in comb)) >= self.config['decadas_min'] and 
                all(v <= self.config['max_por_decada'] for v in CollectionsCounter(min((n - 1) // 10, 3) for n in comb).values())
            ),
